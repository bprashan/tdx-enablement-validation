...
def clone_and_patch_tdx_repository():
    ...
    clone_repo(configuration.canonical_tdx_url, configuration.canonical_tdx_dir, configuration.canonical_tdx_branch)
...
def create_td_image():
    ...
    directory = os.path.expanduser('~') + "/" + configuration.folder_name +"/" + configuration.dir_name + "/" + configuration.canonical_tdx_dir +"/guest-tools/image"
    print(directory)
    script = "sudo ./create-td-image.sh -v " + configuration.os_version

    # Run the script with sudo
    run_command([script], cwd=f"{directory}", shell=True)
    set_environment_variables(key="BASE_IMAGE_PATH", data=f"{directory}/tdx-guest-ubuntu-24.04-generic.qcow2")
    set_environment_variables(key="ENCRYPTED_IMAGE_PATH", data=f"{directory}/tdx-guest-ubuntu-encrypted.img")